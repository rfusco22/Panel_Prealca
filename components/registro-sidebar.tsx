...
import {
  Building2, Package, Users, UserCircle, HardHat, AlertTriangle,
  FileText, ShoppingCart, LayoutDashboard, LogOut, X
} from "lucide-react";
...
const menuItems = [
  { id: "dashboard", label: "Dashboard", icon: LayoutDashboard, href: "/registro" },
  ...
  { id: "proveedores", label: "Proveedores", icon: Package, href: "/registro/proveedores" },
      ...
      >
        ...
        <nav className="flex-1 px-4 space-y-2 mt-6 overflow-y-auto overflow-x-hidden">
          ...
          {menuItems.map((item) => {
            ...
            const isActive = pathname.startsWith(item.href) && (pathname === item.href || item.href !== "/registro");
            return (
        ...
        <div className={`p-4 bg-slate-900/50 border-t border-slate-800/50 mt-auto ${isCollapsed ? 'flex flex-col items-center gap-4' : ''}`}>
          <div className={`flex items-center gap-3 mb-2 ${isCollapsed ? 'justify-center px-0' : 'px-2 py-3'}`}>
            <div className={`h-10 w-10 rounded-xl ${bgColor} flex items-center justify-center text-sm font-bold text-white shrink-0`}>RG</div>
            {!isCollapsed && (
              <div className="flex-1 overflow-hidden whitespace-nowrap">
                <p className="text-sm font-bold text-white truncate">Registro</p>
                <p className={`text-[10px] ${accentColor} uppercase font-bold tracking-wider`}>Rol de Registro</p>
              </div>
            )}
          </div>
          ...
          >
                ...
                return (
                  <Link key={item.id} href={item.href} onClick={onCloseMobile}>
                    <div className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${
                      isActive ? `bg-white/5 text-blue-400 font-semibold` : "hover:bg-slate-800/50 hover:text-white"
                    }`}>
                      <Icon size={20} className={`${isActive ? "text-blue-400" : "text-slate-400"} shrink-0`} />
            ...
            <div className="p-4 bg-slate-900/50 border-t border-slate-800/50 mt-auto">
              <div className="flex items-center gap-3 mb-2 px-2 py-3">
                <div className="h-10 w-10 rounded-xl bg-blue-500 flex items-center justify-center text-sm font-bold text-white shrink-0">RG</div>
                <div className="flex-1 overflow-hidden">
                  <p className="text-sm font-bold text-white truncate">Registro</p>
                  <p className="text-[10px] text-blue-400 uppercase font-bold tracking-wider">Rol de Registro</p>
                </div>
              </div>
