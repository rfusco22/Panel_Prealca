...
export async function POST(request: Request) {
  try {
    ...
    if (!clienteId || !productoId || !cantidadM3) {
      return NextResponse.json({ error: 'Cliente, producto y cantidad son requeridos' }, { status: 400 });
    }

    const result: any = await query(`
      INSERT INTO pedidos (cliente_id, producto_id, cantidad_m3, notas, usuario_id)
      VALUES (?, ?, ?, ?, ?)
    `, [clienteId, productoId, cantidadM3, notas || null, session.userId]);
...
export async function PUT(request: Request) {
  try {
    ...
    await query(`
      UPDATE pedidos SET estado = COALESCE(?, estado), notas = COALESCE(?, notas) WHERE id = ?
    `, [estado || null, notas !== undefined ? notas : null, id]);
