...
export async function GET() {
  try {
    const facturas = await query(`
      SELECT
        f.id,
        f.guia_despacho_id AS guiaDespachoId,
        f.cliente_id AS clienteId,
        c.nombre AS clienteNombre,
        f.forma_pago AS formaPago,
        f.comprobante_retencion AS comprobanteRetencion,
        f.total,
...
export async function POST(request: Request) {
  try {
    ...
    const result: any = await query(`
      INSERT INTO facturas (guia_despacho_id, cliente_id, forma_pago, comprobante_retencion, total, usuario_id)
      VALUES (?, ?, ?, ?, ?, ?)
