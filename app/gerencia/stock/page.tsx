...
export default function GerenciaStockPage() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center min-h-[400px]"><p className="text-slate-500 animate-pulse">Cargando inventario...</p></div>;
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-slate-500 mt-1">Estado del stock por producto y materia prima.</p>
      </div>

      <div className="flex gap-1 bg-slate-100 p-1 rounded-lg w-fit">
        <button onClick={() => setTab('productos')} className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${tab === 'productos' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
          <Box size={14} className="inline mr-1.5" /> Por Producto
        </button>
        <button onClick={() => setTab('materiaPrima')} className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${tab === 'materiaPrima' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
          <Package size={14} className="inline mr-1.5" /> Materia Prima
        </button>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                  ...
                  return (
                    <Fragment key={prod.productoId}>
                      <tr className="hover:bg-slate-50/50 transition-colors">
                        <td className="px-6 py-4"><span className="font-semibold text-slate-900">{prod.productoNombre}</span></td>
                        <td className="px-6 py-4 text-right">
                          <span className={`font-bold ${stock <= 0 ? 'text-red-600' : stock < 200 ? 'text-amber-600' : 'text-emerald-600'}`}>{stock.toFixed(2)} M³</span>
                        </td>
                        <td className="px-6 py-4 text-right text-slate-600">{Number(prod.totalDespachado).toFixed(2)} M³</td>
                        <td className="px-6 py-4 text-center"><span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-bold ${badge.cls}`}>{badge.label}</span></td>
                        <td className="px-2 py-4">
                          {prod.formula?.length > 0 && (
                            <button onClick={() => setExpandedProduct(isExpanded ? null : prod.productoId)} className="p-1 rounded-lg hover:bg-slate-100 transition-colors">
                      ...
                      {isExpanded && prod.formula?.length > 0 && (
                        <tr>
                          <td colSpan={5} className="px-6 py-4 bg-slate-50/80">
                            <div className="ml-8">
                              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Composición de Fórmula</p>
                              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                                {prod.formula.map((f: any, idx: number) => {
                                  const disp = Number(f.disponible);
                                  const req = Number(f.cantidadRequerida);
                                  const color = disp <= 0 ? 'border-red-200 bg-red-50/50' : (req > 0 && disp / req < 1) ? 'border-amber-200 bg-amber-50/50' : 'border-emerald-200 bg-emerald-50/50';
                                  return (
                                    <div key={idx} className={`rounded-lg border p-3 ${color}`}>
                                      <div className="font-semibold text-xs text-slate-900">{f.agregadoNombre}</div>
                                      <div className="text-[11px] text-slate-500 mt-1">Requiere: <span className="font-bold">{req}</span> {f.unidadMedida}/M³</div>
                                      <div className="text-[11px] text-slate-500">Disponible: <span className="font-bold">{disp.toFixed(2)}</span> {f.unidadMedida}</div>
                                    </div>
                                  );
                                })}
                              </div>
                ...
                {productos.length === 0 && (
                  <tr><td colSpan={5} className="px-6 py-12 text-center"><Package size={32} className="text-slate-300 mx-auto mb-2" /><p className="text-sm text-slate-400">No hay productos registrados</p></td></tr>
                )}
              </tbody>
            </table>
          </div>
                ...
                {materiaPrima.map((mp) => {
                  ...
                  return (
                    <tr key={mp.agregadoId} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-6 py-4"><div className="font-semibold text-slate-900">{mp.agregadoNombre}</div><div className="text-xs text-slate-400">{mp.unidadMedida}</div></td>
                      ...
                      <td className="px-6 py-4 text-right"><span className={`font-bold ${disp <= 0 ? 'text-red-600' : disp < 100 ? 'text-amber-600' : 'text-emerald-600'}`}>{disp.toFixed(2)}</span></td>
                      <td className="px-6 py-4 text-center"><span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-bold ${badge.cls}`}>{badge.label}</span></td>
                    </tr>
                  );
                })}
                {materiaPrima.length === 0 && (
                  <tr><td colSpan={5} className="px-6 py-12 text-center"><Package size={32} className="text-slate-300 mx-auto mb-2" /><p className="text-sm text-slate-400">No hay materia prima registrada</p></td></tr>
                )}
              </tbody>
            </table>
          </div>
